...
def extract_symptoms_from_text(text, known_symptoms):
    ...
    """
    # Define stopwords to filter out non-symptom words
    stopwords = {
        "i", "have", "been", "a", "and", "in", "the", "of", "to", "feel", "has", "with", "on", "at", "for",
        "experiencing", "am", "is", "are", "was", "were", "my", "me", "since", "from", "having"
    }
    ...
    for candidate in candidates:
        ...
        for symptom in known_symptoms:
            ...
            if score > best_score and score > 70:  # Stricter threshold for better matching
                best_score = score
                best_match = symptom
        if best_match:
...
def main():
    ...
    while True:
        # If text is not provided via command line, prompt the user
        if args.text is None:
            ...
            text = args.text  # Use command-line input for the first iteration
            args.text = None  # Clear args.text to switch to interactive mode

        # Extract symptoms from free text
        print("\nExtracting symptoms from text...")
        processed_symptoms = extract_symptoms_from_text(text, known_symptoms)
